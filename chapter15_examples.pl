...
{# УСЛОВИЯ when С НЕСКОЛЬКИМИ ЭЛЕМЕНТАМИ
    ...
    # Иногда требуется выполнить однотипную обработку нескольких
    # элементов, но given каждый раз получает только один элемент.
    # Конечно, можно заключить given  в цикл foreach.
        # foreach my $name ( @names ) {
            # given( $name ) {...}
        # }
    # На этот раз мы определяем псевдоним для текущего элемента
    # @names только для того, чтобы конструкция given могла определить
    # для него другой псевдоним! Для перебора нескольких элементов ключевое
    # слово given  вообще не потребуется. Пусть цикл foreach  сам поместит
    # текущий элемент в $_. Если вы хотите использовать умные сравнения,
    # текущий элемент долQ жен храниться в $_.
    use experimental 'switch';
    foreach ( @ARGV ) { # Не используем именованную переменную!
    when( /fred/i ) {say '6 Name has fred in it ', "[$_]"; continue }
    when( /^Fred/ ) { say '6 Name starts with Fred', "[$_]"; continue }
    when( 'Fred'  ) { say '6 Name is Fred', "[$_]"; }
    default         { say "6 I don't see a Fred", "[$_]"; }
    }
